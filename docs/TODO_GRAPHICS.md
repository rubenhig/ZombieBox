...
- [x] **Assets Acquisition:**
    - [x] Obtain a TileSet (recommendation: Kenney Topdown Shooter).
    - [x] Place assets in `assets/sprites/environment/`.
- [x] **TileMap Setup:**
    - [x] Create a `TileMapLayer` node in `GameSession/World`.
    - [x] Configure TileSet resource (define physics collision for walls).
    - [x] Paint the ground (Layer 0) and walls (Layer 1).
- [x] **Navigation Integration:**
    - [x] Bake the `NavigationRegion2D` to match the new TileMap layout (walkable areas vs walls).
    - [x] Remove the hardcoded debug polygon.

## Phase 2: Characters & Projectiles (Visual Identity)
**Goal:** Replace `icon.svg` with animated sprites.

- [x] **Player:**
    - [x] Import Player spritesheet (Idle, Run, Shoot).
    - [x] Replace `Sprite2D` with `AnimatedSprite2D` or `AnimationPlayer` in `player.tscn`.
    - [x] Create `PlayerVisuals.cs` component (decoupled from logic) to handle animation states based on `PlayerInput`.
- [x] **Enemies:**
    - [x] Import Zombie spritesheet.
    - [x] Update `enemy.tscn` visuals.
    - [ ] Add simple "Hit Flash" shader for damage feedback.
- [ ] **Projectiles (Bullets):**
    - [ ] Import bullet sprites (bullet casing, trail).
    - [ ] Create `BulletVisuals.cs` to handle rotation alignment and trail effects.
    - [ ] Add `Line2D` trail for visual feedback of trajectory.
